...
test('completes a walkthrough and persists its shopping-list result', async ({ page }) => {
  ...
  const persisted = await page.evaluate(() => ({
    shoppingList: JSON.parse(localStorage.getItem('shopping_list') ?? '[]'),
    walkthroughCount: JSON.parse(localStorage.getItem('walkthrough_count') ?? '0'),
    lastWalkthroughAt: JSON.parse(localStorage.getItem('last_walkthrough_at') ?? '0'),
